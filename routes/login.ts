...
export function login () {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const safeEmail = req.body.email ? req.body.email.replace(/'/g, "''") : ''
    models.sequelize.query(`SELECT * FROM Users WHERE email = '${safeEmail}' AND password = '${security.hash(req.body.password || '')}' AND deletedAt IS NULL`, { model: UserModel, plain: true }) // vuln-code-snippet vuln-line loginAdminChallenge loginBenderChallenge loginJimChallenge
